...
def _score_dir(model, device, root: Path, batch_size: int = 64):
    tf = build_eval_transform("clean")
    autocast = (
...
def main() -> int:
    ...
    gen_s, gen_f = _score_dir(model, device, Path(args.genval))
    cal_s, cal_f = _score_dir(model, device, Path(args.calval))
    ...
    for spec in args.also:
        nm, d = spec.split("=", 1)
        s, f = _score_dir(model, device, Path(d))
