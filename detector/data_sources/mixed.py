"""Mixed data source: local PS5 data plus a sample of SID_Set, combined
into one training pool.
...
generator diversity. This module is the fix -- mixing in real SID_Set
training images (its "train" shards; "validation" shards are deliberately
left untouched here since they were already used for the held-out
cross-dataset evaluation above, and pulling from them here would leak
eval data into training).
"""
...
from . import local, sid_set_stream
...
def ingest(settings: dict[str, Any]) -> tuple[Dataset, Dataset, dict[str, Any]]:
    ...
    train_dataset = ConcatDataset([local_train, sidset_train])
    val_dataset = ConcatDataset([local_val, sidset_val])

    info = {
        "train_count": local_info["train_count"] + sidset_info["train_count"],
        "val_count": local_info["val_count"] + sidset_info["val_count"],
        "manifest": f"{local_info['manifest']} + {sidset_info['manifest']}",
