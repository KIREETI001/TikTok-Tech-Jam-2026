...
def _score(model, device, paths, condition, batch_size=32):
...
def run(checkpoint: str | Path, real_dir: Path, fake_dir: Path, batch_size: int = 32):
    ...
    for condition in EVALUATION_CONDITIONS:
        real_scores = _score(model, device, reals, condition, batch_size)
        fake_scores = _score(model, device, fakes, condition, batch_size)
