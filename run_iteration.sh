#!/usr/bin/env bash
# One iteration of the train / calibrate / eval loop.
# Env: PYTHON, TTJ_DATA (materialised-data root), HF_HOME, SYCL_CACHE_DIR.
...
#    (genval fits it, calval picks the rule) 
...
run pipeline.py evaluate --device "${DEVICE:-auto}" --run-dir "runs/$RUN" 2>&1 | filt
